...
def generate_strings():
    ...
    for _ in range(n):
        while True:
            character = random.randint(35, 123)  # If the chosen character is not in the excluded list and not already in the string, it is added.
...
def create_images(str_combination: str):
    ...
    img = Image.new("L", (im_width, im_height),color[0])  # Image object of the corresponding size, color, and mode ('L' mode is monochrome).
    ...
    for _ in range(random.randint(3,7)):  # A random number of lines is generated.
        draw.line((random.uniform(0, im_width), random.uniform(0, im_height), random.uniform(0, im_width), random.uniform(0, im_height)), fill=color[1], width=random.randrange(3, 5))
